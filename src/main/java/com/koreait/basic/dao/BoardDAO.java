...
public class BoardDAO {
    ...
    private static String getSearchWhereString(BoardDTO param){
        if(param.getSearchText() != null && !"".equals(param.getSearchText())){
            switch (param.getSearchType()){
                case 1: //제목
                    return String.format(" WHERE title LIKE '%%%s%%'", param.getSearchText());
                case 2: //내용
                    return String.format(" WHERE ctnt LIKE '%%%s%%'", param.getSearchText());
                case 3: //제목및내용
                    return String.format(" WHERE title LIKE '%%%s%%' OR ctnt LIKE '%%%s%%'", param.getSearchText(), param.getSearchText());
                case 4: //글쓴이
    ...
    public static BoardVO selBoardDetail(BoardDTO param){
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sql = "SELECT A.title, A.ctnt, A.hit, A.rdt, A.writer, B.nm as writerNm FROM t_board A INNER JOIN t_user B ON A.writer = B.iuser WHERE A.iboard = ?";
        try{
            con = DbUtils.getCon();
            ps = con.prepareStatement(sql);
